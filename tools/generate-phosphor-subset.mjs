...
files.forEach((file) => {
  ...
  contents.match(/\bph-[a-z0-9-]+\b/g)?.forEach((icon) => {
    if (!['ph-bold', 'ph-fill'].includes(icon)) requested.add(icon);
  });
});
...
const supported = [...requested].filter((icon) => mappings.has(icon)).sort();
const missing = [...requested].filter((icon) => !mappings.has(icon)).sort();
const codepoints = supported.map((icon) => `U+${mappings.get(icon).toUpperCase()}`);
...
if (subset.status !== 0) {
  process.stderr.write(subset.stderr || subset.stdout || 'pyftsubset failed.');
...
${missing.length ? `/* Not present in @phosphor-icons/web/bold and intentionally left unmapped: ${missing.join(', ')}. */\n` : ''}@font-face {
  ...
  src: url("/phosphor-bold-subset.woff?v=2") format("woff");
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}
...
${supported.map((icon) => `.ph-bold.${icon}:before { content: "\\${mappings.get(icon)}"; }`).join('\n')}
`;

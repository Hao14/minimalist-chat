...
The AI workspace also shows the website's bundled 10 / 40 / 40 routing catalog:
protected Ollama, Cloudflare Workers AI (`@cf/qwen/qwen3-30b-a3b-fp8`), and Groq
(`openai/gpt-oss-20b`). This is read-only release metadata, not a remote
provider-control surface. The remote agent does not send cloud API keys,
Firebase configuration, provider leases, prompts, or provider availability to
the client, and the hosted model IDs never become local Ollama install targets.
...
- Website provider routes shown in the client are bundled audit metadata; provider configuration and secrets remain server-owned.
